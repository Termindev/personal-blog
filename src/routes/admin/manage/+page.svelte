...
{:then articles}
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 p-4">
    {#each articles as article}
      <!-- TODO: Extract into own component for cleaner code -->
      <div
        class="card bg-base-100 shadow-md glow {article.visible
          ? ''
          : 'opacity-50'}"
      >
        <div class="card-body">
          ...
          {#if article.reviewable}
            <a href="/blog/{article.id}" class="link font-bold">Review link</a>
          {/if}
          <!-- Languages + Actions -->
